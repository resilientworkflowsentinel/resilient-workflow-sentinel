...
def render_task_queue(tasks: list):
    """Render queue tasks - clean lines, no expansion"""
    if not tasks:
    ...
    for task in tasks[:50]:  # Show up to 50
        ...
        with ui.card().classes('w-full p-3 mb-2 hover:bg-gray-50'):
            with ui.row().classes('w-full items-center gap-3'):
                ui.label(task_id).classes('font-mono font-semibold text-blue-600 w-32')
                ui.label(task_text[:100] + ('...' if len(task_text) > 100 else '')).classes('text-sm flex-grow')

def render_assignment(assignment: Dict[str, Any]):
    """Render assignment - show original task text from cache and processing duration"""
    ...
    # Show ORIGINAL task text (from cache), truncated if too long
    display_text = task_text[:120] + '...' if len(task_text) > 120 else task_text
    
    # If text is empty, show message
    if not display_text:
        display_text = "⚠️ No task text available"
    
    # Simple card - no expansion
    card = ui.card().classes('w-full mb-2').style(f'border-left: 4px solid {border_color};')
        ...
        with ui.row().classes('items-center gap-4'):
            ...
            with ui.column().classes('flex-grow'):
                ...
                with ui.row().classes('items-center gap-2 mt-2'):
                    ...
                    ui.link('→', 'https://github.com/your-repo', new_tab=True).classes('text-white text-lg font-bold underline')
        ...
        with ui.row().classes('w-full px-6 items-end justify-between gap-6'):
            ...
            with ui.column().classes('flex-grow justify-end items-center pb-1'):
                with ui.row().classes('items-center gap-1'):
                    ...
                    ui.link('→ gumroad.com/l/sentinel-pro', 'https://gumroad.com/l/sentinel-pro', new_tab=True).classes('text-blue-400 text-sm underline')
            ...
            with ui.column().classes('items-end gap-2'):
                ...
                with ui.row().classes('items-center gap-4'):
                    ...
                    with ui.row().classes('items-center gap-2'):
                        ...
                        ui.link('GitHub', 'https://github.com/your-repo', new_tab=True).classes('text-white text-sm font-semibold')
